function [ re ] = judgein( countnum,cuecount,cuen,BSgain,multigain,D2Dgain,D2DarrayGroup,CUEarrayGroup,powervalue,CUEBSarrayGroup ,CUEnum,D2Dnum,N_0,pd)
...
for j=1:D2Dnum
    if CUEarrayGroup(countnum,cuen)==D2DarrayGroup(countnum,j)
        %同信道的D2D计算
        d2dnoise1=d2dnoise1+pd(1,j)*(multigain(cuecount,j)^2);
        d2dnoise2=d2dnoise2+pd(1,j)*(multigain(cuen,j)^2);
    end
end
re=0;
