function [newvalues,newpc,newpd] = cvx_calculate2(BSgain,multigain,D2Dgain,CUEBSarrayGroup,CUEarrayGroup,D2DarrayGroup,BSnum,N_0,CUEnum,D2Dnum,channelnum,vpm,vdm,vtm,vbm,ub,vb,P_BS_1,P_BS_2,t,fpc,ud,vdd,fpd)
           ...
           for bs=1:BSnum
               for jj=1:CUEnum
                   ...
                   if vbm(bs,jj)==1&&vpm(m,jj)==1
                       cuenoise=2^pc(1,jj)*(BSgain(bs*2,w)^2)+cuenoise;
                   end
               end
           end
           for z1=1:D2Dnum
                                            ...
                                            for j=k+1:count
                                                ...
                                                for one=1:D2Dnum%这里求同一信道上面的D2d干扰
                                                    if vdm(j1,one)==1
                                                        ...
                                                        d2d2k=d2d2k+BSgain(i1*2-1,hk)^2*multigain(hj,one)^2*log(2)*(log(2^pd(1,one))-log(fpd(1,one)));
                                               ...
                                               for bbs=1:BSnum%不同基站同一信道的干扰
                                                   if bbs~=i1
                                                        for s=1:CUEnum%k的增益
                                                         ...
                                                         scuek=scuek+BSgain(i1*2-1,hk)^2*BSgain(bbs*2-1,hk)^2*pm;
                                                   end
                                               end
                                               bcuej=0;
                                               pmc=0;
                                                   ...
                                                   if bbc~=i1
                                                        for s1=1:CUEnum%k的增益
                                                            if vbm(bbc,s1)==1&&vpm(j1,s1)==1
                                                                pmc=pmc+log(2)*(log(2^pc(1,s1))-log(fpc(1,s1)));
                                                                 pmb=pmb+fpc(1,s1);
%                                                                  pmb=pmb+2^pc(1,s1);
                                                            end
                                                        end
                                               ...
                                               scuek-(BSgain(i1*2-1,hk)^2-BSgain(i1*2-1,hj)^2)*N_0^2/2<=bcuej;
                                               d2d1k-(BSgain(i1*2-1,hk)^2-BSgain(i1*2-1,hj)^2)*N_0^2/2<=d2d2k;
                                            end
                                    end
                                end
                            end
